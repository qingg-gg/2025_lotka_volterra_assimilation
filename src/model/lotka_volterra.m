...
% 定義：兔子、狐狸、熊的數量變化關係
% 公式：
%   (1) rate_x = alpha_x * x * (1 - x / K) - beta_y * (x / (1 + delta_y * x)) * y
%   (2) rate_y = alpha_y * (beta_y * x_past / (1 + delta_y * x_past)) * y_past - gamma_y * y - beta_z * (y / (1 + delta_z * y)) * z
%   (3) rate_z = alpha_z * (beta_z * y_past / (1 + delta_z * y_past)) * z_past - gamma_z * z
% ================================================================================================================================= %
...
alpha_y = growth_rate_fox(t, beta_y, delta_y, gamma_y);
alpha_z = growth_rate_bear(t, beta_z, delta_z, gamma_z);
K = environment_capacity(t);                            % 環境負載力

rate = zeros(size(now_state));
rate(1) = alpha_x * x_now * (1 - x_now / K) - beta_y * (x_now / (1 + delta_y * x_now)) * y_now;
rate(2) = alpha_y * (beta_y * x_past / (1 + delta_y * x_past)) * y_past - gamma_y * y_now - beta_z * (y_now / (1 + delta_z * y_now)) * z_now;
rate(3) = alpha_z * (beta_z * y_past / (1 + delta_z * y_past)) * z_past - gamma_z * z_now ;
